...
\paragraph{Bước 3 --- giải $w$ bằng hàm riêng.} $\cos(cx)$ là hàm riêng của $\partial_{xx}$ với trị $-c^2$
($\partial_{xx}\cos cx=-c^2\cos cx$). Thử $w=A(\tau)\cos(cx)$ vào $w_\tau=w_{xx}$: $A'=-c^2A$, $A(s)=e^{-s}$
$\Rightarrow A(\tau)=e^{-s}e^{-c^2(\tau-s)}$. Tại $\tau=t$:
\[
  w(x,t;s)=e^{-s}\,e^{-c^2(t-s)}\cos(cx).
\]
